...
if (process.env.VCAP_SERVICES) {
  ...
  redisconf = (redisconf['redis-2.4'] || redisconf['redis'])[0];
  config.redis.host = redisconf.credentials.host;
  config.redis.port = redisconf.credentials.port;
  config.redis.pass = redisconf.credentials.password;
}
...
RedisGameClient.prototype.updateGame = function(game, callback, skipLock) {
    ...
    function performUpdate() {
        client.hset(GAMES_KEY, game.gameId, JSON.stringify(game),
        function(err, data) {
            if (!skipLock) {
...
RedisGameClient.prototype.postScores = function(players, callback) {
    ...
    function postScore(who, score) {
        client.zscore(HIGHSCORE_KEY, who,
        function(err, data) {
            if (err) {
                return;
            }
...
RedisGameClient.prototype.loadScores = function(callback) {
    client.zrevrangebyscore(HIGHSCORE_KEY, '+inf', '-inf', 'withscores', 'limit', 0, 100,
    function(err, data) {
        if (err) {
            callback(err);
        }
